 ...
 */
public class JSpellForm extends BasicContainerPanel implements ActionListener, ListSelectionListener {
    ...
    /** Helper method to create a BasicButton with a command, a text label and a listener*/
    private static final BasicButton createButton(String command, String text, ActionListener listener) {
	BasicButton btn = new BasicButton(text);
    ...
    protected BasicPanel makeEastPanel() {
	BasicPanel jPanel1 = new BasicPanel();
	BoxLayout layout = new BoxLayout(jPanel1, BoxLayout.Y_AXIS);
	jPanel1.setLayout(layout);
	BasicButton ignoreBtn = createButton(IGNORE_CMD, messages.getString(IGNORE_CMD), this);
	ignoreBtn.setMaximumSize(new Dimension(Short.MAX_VALUE, Short.MAX_VALUE));
	jPanel1.add(ignoreBtn);
	BasicButton ignoreAllBtn = createButton(IGNOREALL_CMD, messages.getString(IGNOREALL_CMD), this);
	ignoreAllBtn.setMaximumSize(new Dimension(Short.MAX_VALUE, Short.MAX_VALUE));
	jPanel1.add(ignoreAllBtn);
	BasicButton addBtn = createButton(ADD_CMD, messages.getString(ADD_CMD), this);
	addBtn.setMaximumSize(new Dimension(Short.MAX_VALUE, Short.MAX_VALUE));
	jPanel1.add(addBtn);
	BasicButton changeBtn = createButton(REPLACE_CMD, messages.getString(REPLACE_CMD), this);
	changeBtn.setMaximumSize(new Dimension(Short.MAX_VALUE, Short.MAX_VALUE));
	jPanel1.add(changeBtn);
	BasicButton changeAllBtn = createButton(REPLACEALL_CMD, messages.getString(REPLACEALL_CMD), this);
	changeAllBtn.setMaximumSize(new Dimension(Short.MAX_VALUE, Short.MAX_VALUE));
	jPanel1.add(changeAllBtn);
	BasicButton cancelBtn = createButton(CANCEL_CMD, messages.getString(CANCEL_CMD), this);
	cancelBtn.setMaximumSize(new Dimension(Short.MAX_VALUE, Short.MAX_VALUE));
	jPanel1.add(cancelBtn);
	return jPanel1;
    }

    protected BasicPanel makeCentrePanel() {
	BasicPanel jPanel2 = new BasicPanel();
	jPanel2.setLayout(new BoxLayout(jPanel2, BoxLayout.Y_AXIS));
	BasicLabel lbl1 = new BasicLabel(messages.getString(INVALIDWORD_RES));
	jPanel2.add(lbl1);
	checkText = new JTextArea();
	jPanel2.add(new BasicScrollPane(checkText));
	BasicLabel lbl2 = new BasicLabel(messages.getString(SUGGESTIONS_RES));
	jPanel2.add(lbl2);
	suggestList = new JList();
	suggestList.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
	jPanel2.add(new BasicScrollPane(suggestList));
