...
public class PanelPagoAnticipos extends BasicPanel{
    ...
    private BasicButton btnPagoContado = new BasicButton("Pago de Contaodo");
	...
	separador1.setBounds(new Rectangle(0, 435, 615, 5));
	...
	rbtnLibreDeuda.setBounds(new Rectangle(220, 355, 75, 20));
        btnImprimirEstadoDeuda.setBounds(new Rectangle(540, 445, 75, 20));
	...
	btnRegistrarBoleta.setText("Reg. Boleta");
	btnRegistrarBoleta.setBounds(new Rectangle(10, 445, 120, 25));
	...
	btnPagoContado.setText("Pago Contado");
	btnPagoContado.setBounds(new Rectangle(140, 445, 130, 25));
	...
	btnPlanDePago.setText("Plan de Pago");
	btnPlanDePago.setBounds(new Rectangle(280, 445, 120, 25));
	...
	btnLibreDeuda.setText("Libre Deuda");
	btnLibreDeuda.setBounds(new Rectangle(410, 445, 120, 25));
        ...
        if(coordinador.getBien().puedeHacerPagoContado(coordinador.getImpuesto())){
            ...
            if ( deudaActual > 0.00) {
                ...
                if (coordinador.getPagoAnticipos().getMontoTotal() > 0.0) {
                    ...
                    Advisor.messageBox("No se puede generar un pago de contado a una deuda de $ "+ coordinador.getPagoAnticipos().getMontoTotal(), "Error");
                }
            } else {
                Advisor.messageBox("No se puede generar un pago de contado por que no tiene deuda ", "Aviso");
            }
    ...
    private void generarLibreDeuda(){
	if(LibSQL.getBoolean("taxes.getLibreDeuda",""+ coordinador.getImpuesto().getTipoImpuesto().getIdTipoImpuesto() +","+ coordinador.getBien().getIdBien())){
            if (Advisor.question("Aviso", "¿Está seguro de generar el libre deuda?")) {
	    ...
	    Advisor.messageBox("No puede generar libre deuda.","Aviso!");
	}
    }
	...
	if (boletaContribucion.saveData() > 0)  {       
	   ...
	   Advisor.messageBox("Se generó correctamete el libre deuda con la boleta Nº "
                                + boletaContribucion.getAnio() 
                                + "-" +boletaContribucion.getNumeroFormateado() ,"Aviso");
	} else  {
