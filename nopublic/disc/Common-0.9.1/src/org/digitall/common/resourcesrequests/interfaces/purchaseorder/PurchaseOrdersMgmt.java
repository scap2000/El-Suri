...
public class PurchaseOrdersMgmt extends BasicPrimitivePanel {
    ...
    private PurchaseOrderGenerateList purchaseOrderGenerateList;
    ...
    private boolean registerNotifications() {
	...
	try {
		...
		notification.setText("Se generó la Orden de Compra Nº " + purchaseOrder.getNumber() + ", referida al Pedido de Materiales Nº " + resourcesRequest.getNumber());
		notification.setSubject("Nueva Orden de Compra Nº " + purchaseOrder.getNumber() + " (Pedido de Materiales Nº " + resourcesRequest.getNumber() + ")");
	...
	if (validateData()) {
	    if (saveData()) {
		purchaseOrderGenerateList.refresh();
	...
	if (Advisor.question("Grabar Orden de Compra", "¿Está seguro de grabar la Orden de Compra por un monto de " + tfTotalAmount.getTextField().getText() + "?")) {
		...
		if (purchaseOrder.saveData() >= 0) {    // si se grabó el enc. ==> grabar el detalle de la Orden de Compra
			...
			if (purchaseOrder.addBudgetMovementByPurchaseOrder() > 0) {
			    ...
			    Advisor.messageBox("Se produjo un error al registrar los movimientos presupeustarios\nNo se pudo generar la Orden de Compra", "Error");
    ...
    public void setPurchaseOrderGenerateList(PurchaseOrderGenerateList purchaseOrderGenerateList) {
	this.purchaseOrderGenerateList = purchaseOrderGenerateList;
    }
	...
	resourcesRequest = new ResourcesRequest(_idResourcesRequest);
	...
	tfTotalAmount.setValue(LibSQL.getDouble("resourcescontrol.getresourcesrequestauthdetailbytotalamount", resourcesRequest.getIdResourcesRequest()));
	detailPanel.refresh(resourcesRequest.getIdResourcesRequest() + ", 'true'");
    }
