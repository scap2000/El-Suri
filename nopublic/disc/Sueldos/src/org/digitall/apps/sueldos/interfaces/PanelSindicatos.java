...
public class PanelSindicatos extends BasicPrimitivePanel {
	...
	if(!contentResguardo.equals(contentActual)){
		...
		if(contentActual.save()){
		    refresh();
		    cargarContent(contentResguardo,false);
		    Advisor.messageBox("Se guardaron los cambios de la pestaña SINDICATOS","Datos grabados");
		}else{
	    ...
	    if(!_exit){
		Advisor.messageBox("No se realizaron cambios en la pestaña SINDICATOS","Sin cambios");	
	    }
	}
    }
