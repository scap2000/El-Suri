...
public class ConceptosNovedades
{
	...
	if (idconceptoreferencia == -1){
	    result = LibSQL.getInt("sueldos.addConceptoNovedades",params);              
	} else {            
	    result = LibSQL.getInt("sueldos.setConceptoNovedades",params);
	}               
	return result;
    }
	...
	ResultSet data = LibSQL.exFunction("sueldos.getConceptoNovedades", params);
	try {
	    if (data.next()) {
