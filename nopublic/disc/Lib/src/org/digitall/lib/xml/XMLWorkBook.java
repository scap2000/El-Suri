...
public class XMLWorkBook {
    ...
    private int sheetPaint = -1;
    private Vector[] widthColumnVector;
	...
	try {
	    ...
	    org.digitall.lib.components.Advisor.messageBox("Error al obtener la Foto", "Error XMLWorkBook");
	    x.printStackTrace();
	}
	return "";
    }
	...
	widthColumnVector = new Vector[nroHojas];
	titleStyle = new String[nroHojas];
	for (int i = 0; i < _nroHojas; i++) {
	    titleStyle[i] = "Normal8";
	}
    }
    ...
    public void setTitleStyle(int _nroHoja, String _titleStyle) {
	if (_nroHoja < nroHojas) {
	    titleStyle[_nroHoja] = _titleStyle;
	} else {
	    org.digitall.lib.components.Advisor.messageBox("El índice especificado supera el número de Hojas", "Error de XMLWorkBook");
	}
    }
    ...
    public void setWidthColumn(int _nroHoja, Vector _widthColumnVector) {
	if (_nroHoja < nroHojas) {
	    widthColumnVector[_nroHoja] = _widthColumnVector;
	} else {
	    org.digitall.lib.components.Advisor.messageBox("El índice especificado supera el número de Hojas", "Error de XMLWorkBook");
	}
    }
	...
	//Nombre de la Hoja
	//Inicio de la Hoja
	String nameSheet = nombreHoja[_nroHoja];
	if (nameSheet.length() > 30)
	    nameSheet = nameSheet.substring(0, 27) + "...";
	write("<!-- Inicio del cuerpo -->\n" 
	    + " <Worksheet ss:Name=\"" + nameSheet + "\">\n" 
		+ "   <Names>\n" 
	...
	for (int i = 0; i < cantidadColumnas[_nroHoja]; i++) {
	    if (widthColumnVector[_nroHoja].size() > 0) {
		iniciotabla += "       <Column ss:Width=\"" + widthColumnVector[_nroHoja].elementAt(i) + "\"/>\n";
	    } else {
		iniciotabla += "       <Column ss:Width=\"" + 216 + "\"/>\n";
	    }
	}
	/*********************************************** Titulo de la hoja: '_nroHoja' *********************************************/
	iniciotabla += "   <Row ss:Height=\"16.5\">\n";
	iniciotabla += "    <Cell ss:MergeAcross=\"" + (cantidadColumnas[_nroHoja] - 1) + "\" ss:StyleID=\"" + titleStyle[_nroHoja] + "\"><Data ss:Type=\"String\">" + tituloHoja[_nroHoja] + "</Data><NamedCell ss:Name=\"_FilterDatabase\"/></Cell>\n";
	iniciotabla += "   </Row>\n";
	/**************************************************************************************************************************/
	iniciotabla += "   <Row ss:Height=\"16.5\">\n";
	//Cabecera
	...
	String fila = "";
	try {
	    ResultSet Resulx = org.digitall.lib.sql.LibSQL.exQuery(_query);
	    int filas = 0;
	    while (Resulx.next()) {
		fila = "   <Row ss:Height=\"16.5\">\n";
		for (int i = 0; i < cantidadColumnas[_nroHoja]; i++) {
		    if (colorColumn == i + 1 && sheetPaint == _nroHoja) {
			fila += "    <Cell ss:StyleID=\"Priority" + Resulx.getString(i + 1) + "\"><Data ss:Type=\"" + vectorTipoDatos[_nroHoja].elementAt(i) + "\"></Data></Cell>\n";
		    } else if (colorColumn != i + 1) {
	    ...
	    org.digitall.lib.components.Advisor.messageBox(x.getMessage(), "Error XMLWorkBook");
	    x.printStackTrace();
	}
    }

    public void setAppendStyle(String _appendStyle) {
	appendStyle = _appendStyle;
    }

    public void setColorColumn(int _sheetPaint, int _colorColumn) {
	colorColumn = _colorColumn;
	sheetPaint = _sheetPaint;
    }
