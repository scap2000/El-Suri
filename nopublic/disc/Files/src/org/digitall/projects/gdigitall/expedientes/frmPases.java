...
//???import org.opsalta.sistemas.SOPSalta.Personas;
public class frmPases extends BasicPrimitivePanel implements ActionListener, KeyListener {
    ...
    private JButton boficemi = new JButton(); //new ImageIcon(OP_Proced.class.getResource("iconos/16x16/buscar.gif")));
    ...
    private JButton bemisor = new JButton(); //new ImageIcon(OP_Proced.class.getResource("iconos/16x16/buscar.gif")));
    ...
    private JButton breceptor = new JButton(); //new ImageIcon(OP_Proced.class.getResource("iconos/16x16/buscar.gif")));
    private JButton boficrec = new JButton(); //new ImageIcon(OP_Proced.class.getResource("iconos/16x16/buscar.gif")));
    private JButton bactivar = new JButton(); //new ImageIcon(OP_Proced.class.getResource("iconos/16x16/activar.gif")));
    private JButton beditor = new JButton(); //new ImageIcon(OP_Proced.class.getResource("iconos/16x16/editor.gif")));
   ...
   */
    public frmPases(String SQLQuery, String IDInst, String NroExpte, String NroCatastro, String Accion, String Folios, String IDOficRec, String IDReceptor) {
        try {
            Query = SQLQuery;
            /*ESTA VARIABLE ME INDICA SI SE VA A MODIFICAR O AGREGAR UN NUEVO INST LEGAL, O SEA,
                           SI EL TAMAÑO DE ESTA VBLE. ES >0, ENTONCES QUIERE DECIR QUE POSEE UNA CONSULTA,
                          DONDE ESTA CONSULTA ESTA LISTA P SER EJECUTADA Y LLENAR TODOS LOS CAMPOS DEL FORMULARIO*/
            accion = Accion;
            nroexpte = NroExpte;
	...
	jPanel1.add(jcestadop, null);
     ...
     LOS ITEM'S DEL COMBO INDICAN DICHA DESCRIPCION*/
        ...
        jcoficrec.addItemListener(new ItemListener() {
                public void itemStateChanged(ItemEvent evt) {
                    if (evt.getStateChange() == ItemEvent.SELECTED) {
                        // TODO COMO DEBERA TENER ESTA LINEA EN SU CORRESPONDIENTE addItemListener(), ESTO EVITA EL ERROR DE DOBLE CONSULTA
        ...
        if (Query.length() > 0) {		//ESTO SIGNIFICA QUE SE VA A MODIFICAR UN pase
            ...
            if (Reg.next()) {
                ...
                if (!accion.equals(""))/** REGISTRAR RECEPCION DEL PASE DEL DOCUMENTO */
                {
        ...
        } else /** ESTO SI SE VA A AGREGAR UN NUEVO Pase */
        {
            nuevo = true;
            jtidreceptor.setEditable(false);
            breceptor.setEnabled(false);
            ...
            //Proced.CargaCombo(jcestadop,"SELECT descripcion FROM files.tablacombo WHERE combo='jcestadop'","Enviado");
            ...
            sidoficemi = jtidoficemi.getText();
            //AGREGADO POR GERMAN Y SANTIAGO
            OP_Proced.CargaCombo(jcorganismo, "SELECT descripcion FROM files.tiposorganismo WHERE idorganismo=213", "");
            //        jtidemisor.setText(idreceptor);
            // System.out.println("SELECT apellido||', '||nombre FROM personas WHERE alias='"+ Proced.SQLUser +"'");
    ...
    private void ActivaReceptor(String estado, boolean activa) {
        ...
        //  jtobservemis.setEditable(activa);  
    ...
    private void ActivaCampo2(boolean op) {
        //    System.out.println("ActivaCampo2");
        jtidoficemi.setEditable(op);
        jcoficemi.setEnabled(op);
        // jtobservemis.setEditable(op);  
        ...
        try {
            if (e.getSource() == bcancelar) {
                ...
                if ( new Integer(jtfoliose.getText()) > 0) {
                    if (Query.length() > 0)//SE GENERA LA CONSULTA P MODIFICACION
                    {
                        String Where = "";
                        ...
                        if (OP_Proced.exActualizar('m', Q)) {
                            /** CUANDO UN DOCUMENTO TIENE ADJUNTOS O ANEXOS, ESTOS SE MUEVEN EN CONJUNTO, POR LO Q SIGNIFICA Q CUANDO SE REGISTRA UN PASE AL
                   * DOCUMENTO CABECERA SE DEBERA ACTUALIZAR LOS PASES A TODOS LOS DOC ADJUNTADOS/ANEXADOS */
                            ...
                            //ImprimirTextoPase();
                            //this.dispose();
                            parentMain.ActualizaTabla(); /** Hay que ver si es correcta esta linea de codigo */
                             getParentInternalFrame().close();
                        }
                    } else // SE GENERA LA CONSULTA PARA AGREGAR UN 
                    {
                        if (!jtidoficrec.getTexto().equals("0")) {
                            System.out.println("jtidemisor.getTexto(): " + jtidemisor.getTexto());
                           // if (!jtidemisor.getTexto().equals("0")) {
                                if (!jtidoficemi.getTexto().equals(jtidoficrec.getTexto())) {
                                    ...
                                    if (OP_Proced.exActualizar('a', Q)) {
                                        /** ACTUALIZA PASES A LOS DOCUMENTOS ADJUNTADO/ANEXADOS */
                                        ResultSet Reg = OP_Proced.exConsulta("SELECT idinstref FROM files.instxinst WHERE idinst=" + idinst);
                                        while (Reg.next()) {
                                    ...
                                    /** EL ID DE LA OFICINA RECEPTORA = 213 INDICA EL ARCHIVO GENERAL (cod.213), 
                          * EN ESTE CASO SE DEBERA CAMBIAR EL ESTADO DEL DOCUMENTO A 'ARCHIVO GRAL.'*/
                           ...
                           // } else {
                           //     System.out.println("salto por aca!!!");
                           //     OP_Proced.Mensaje("El usuario que inicio sesion no esta correctamente registrado..\nConsultar con el Administrador del Sistema", "Usuario no registrado");
                           // }
                        } else {
                ...
                /*OP_Proced.CentraVentana(oficrec);
                oficrec.setModal(true);*/
                ...
                /*OP_Proced.CentraVentana(oficemi);
                oficemi.setModal(true);*/
                ...
                //???Personas rec = new Personas();
                //???Proced.CentraVentana(rec);
                //???rec.setModal(true);
                //???rec.setVisible(true);
                //???Proced.CargaCombo(jcreceptor,"SELECT apellido||', '||nombre FROM personas WHERE idpersona="+ rec.getIDPersona(),"");
            } else if (e.getSource() == bemisor) {
                //???Personas emi = new Personas();
                //???Proced.CentraVentana(emi);
                //???emi.setModal(true);
                //???emi.setVisible(true);
                //???Proced.CargaCombo(jcemisor,"SELECT apellido||', '||nombre FROM personas WHERE idpersona="+ emi.getIDPersona(),"");
            } else if (e.getSource() == bactivar) {
                /*String usuario = OP_Proced.getSQLUser();
                String clave = OP_Proced.getSQLPass();
                ...
                if (inicio.getValidado() & OP_Proced.getSQLUser().equals("admin_sim")) {
                    jcestadop.setEnabled(true);
                }
                OP_Proced.setSQLUser(usuario);
                OP_Proced.setSQLPass(clave);*/
            } else if (e.getSource() == beditor) {
                Editor editor = new Editor("Texto del Pase", "Correspondiente al Documento Nº " + jtnroexp.getText(), true, "texto_pase" + idinst, nuevo, jtobservemi.getText());
                OP_Proced.CentraVentana(editor);
                editor.setModal(true);
                editor.setVisible(true);
                //jtobservemi.setPage(new URL("file","",Proced.rutaInforme + Proced.separador + "texto_pase"+ idinst +".html"));
                if (editor.getCodigoFuente().length() > 0) {
                    jtobservemi.setText(editor.getCodigoFuente());
                }
                /* System.out.println(jtobservemi.getPage());
        System.out.println(jtobservemi.getText());*/
            }
        } catch (Exception x) {
            x.printStackTrace();
        }
