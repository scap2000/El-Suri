...
public class LayerDraw extends JFrame //implements MouseListener, MouseMotionListener, MouseWheelListener
{
...
//  final Barra progress = new Barra(max, current, this);
  ...
  private JButton capture = new JButton(new ImageIcon(LayerDraw.class.getResource("iconos/consultar.png")));
...
////////////////////// ------------------------- PARA LA DEMO ----------------------------- ///////////////////////
  ...
  public LayerDraw(LayerObject l)
  {
    numLayers = 1;
    layer[0] = l;
    plotear = false;
  ...
  public LayerDraw(MapObject map, BufferedImage bfdImg)
  {
    xOffset = -1;
    yOffset = -1;
    ...
    try 
    {
      Proc.CargaCombo(comboBarrios,"select nombre from barrios where idbarrio in (select idbarrio from gis.barrios) order by nombre","");
  ...
  public LayerDraw(MapObject map)
  {
    xOffset = -1;
    yOffset = -1;
    numLayers = map.getLayerCount();
  ...
  public LayerDraw(MapObject map, BufferedImage bfdImg, JTextField _ubicacion)
  {
...
//      ubicacion.setText("");
  ...
  public LayerDraw(MapObject map, BufferedImage bfdImg, int numcatastro)
  {
    xOffset = -1;
    yOffset = -1;
                  ...
                  try
                  {
                    ...
                    if ((finDrawing) && (!archivo.equals(""))) 
                    {
                      try {
                          ...
                          //File file = new File("http://localhost/tmp/jms/" + archivo);
                          //imagen = ImageIO.read(file);
                          imagen = ImageIO.read(new URL("http://localhost" + archivo));
                          loaded = true;
...
/*    dibujo.addMouseWheelListener(this);*/
    ...
    capture.setBounds(new Rectangle(2, 362, 40, 40));
    capture.setSize(new Dimension(40, 40));
    capture.setPreferredSize(new Dimension(40, 40));
    capture.setMaximumSize(new Dimension(40, 40));
    capture.setMinimumSize(new Dimension(40, 40));
  ...
  synchronized public void rundoImage() 
  {
    tarea = new Thread(new Runnable() {
      public void run() 
      {
        ...
        File raster = new File("/tmp/jms/raster.jpg");
        try {imagen = ImageIO.read(raster);} catch (IOException x){}
        g2d.drawImage(imagen, topx, topy, (int)(fWidth*ScaleX), (int)(fHeight*ScaleX), null);
        for (int i=0; i<numLayers; i++) 
        {
          //progress.
          setMax(0);
          panel.actualizar();
          System.out.println(layer[i].getonoff());
        ...
        {
          // Save as PNG
          DecimalFormat dxf = new DecimalFormat("00000");
          archivo = ruta + "l-" + dxf.format(Math.random()*10000) + ".jpg";
          File file = new File(archivo);
          ImageIO.write(imagen, "jpg", file);
          //progress.
          setStatus("Imagen grabada... Cargando imagen...");
          ImageIcon icono = new ImageIcon(LayerDraw.class.getResource("iconos/grabar.png"));
          statusbtn.setIcon(icono);
          if (catastro > 0) 
          {
            setStatus("Generando informe...");
            reportero.FichaCatastral(catastro, archivo);
          }
        } catch (IOException e) 
        {
          e.printStackTrace();
...
//      String Qwhere = "";
      ...
      Query = "select " + LabelItem + " astext(force_2d(intersection(" + geometry + ", " + extents.getBox3D() + "))) " +  
  ...
  private void drawPolygons(Graphics2D g2d, ResultSet Polygons, boolean withLabels, LayerObject myLayer)
  {
    ...
    try 
    {
      while (Polygons.next()) 
      {
        ...
        polys[0] = getPolyFromPoints(xd, yd, fNumPoints,Polygons.getString("label"));
...
//            int yl = (int)centro.getY();
  ...
  private void drawPolylines(Graphics2D g2d, ResultSet Polylines, boolean withLabels, LayerObject myLayer)
  {
    ...
    try 
    {
      FileWriter polygonFile = new FileWriter("/tmp/poly-calles.txt",false);
...
//      System.out.println(Poly);
  ...
  private void drawPoint(Graphics2D g2d)
  {
    ...
    /*if (ubicacion.getText().length()>0) 
    {
      int indexComa = ubicacion.getText().indexOf(",");
      double ubicX = Double.parseDouble(ubicacion.getText().substring(0,indexComa));
      double ubicY = Double.parseDouble(ubicacion.getText().substring(indexComa+1, ubicacion.getText().length()));
      Point circle = toPixel(ubicX,ubicY);
      g2d.setColor(new Color(255,0,0));
      g2d.fillOval(circle.x, circle.y, 10,10);
      g2d.setColor(new Color(0,0,0));
      g2d.drawOval(circle.x, circle.y, 10,10);
    }*/
  }
...
//    return scaleimg;
  ...
  public Polygon getPolyFromPoints(double[] xp, double[] yp, int cantidad, String label) 
  {
    ...
    try 
    {
      FileWriter polygonFile = new FileWriter("/tmp/poly-" + label + ".txt",false);
      BufferedWriter log = new BufferedWriter(polygonFile);
      for (int i=0; i < cantidad; i++) 
      {
        x[i] = (int)( (xp[i] - xOffset) * Factor);
        y[i] = (int)( fHeight - ((yp[i] - yOffset) * Factor));
        //polygonFile.write("(x,y)[" + i + "]: " + x[i] + "," + y[i] + "\n");
        polygonFile.write("x[" + i + "] = " + x[i] + "; " + "y[" + i + "] = " + y[i] + "; " + "\n");
      }
      polygonFile.close();
    } catch (Exception ex) 
    {
...
//---------------------------------------- AGREGADO PARA EL DEMO DE MAGNUM --------------------------------/

  public void zoomBarrio()
  {
    xOffset = -1;
    yOffset = -1;
    String Query = "";
    ...
    { 
      Connection Con = Proc.CCon();
      Query = "select xmin(box3d(the_geom)),xmax(box3d(the_geom)),ymin(box3d(the_geom)),ymax(box3d(the_geom)) from gis.barrios where idbarrio = public.barrios.idbarrio and public.barrios.nombre = '" + comboBarrios.getSelectedItem().toString() +"'";
      ResultSet extBarrio = Proc.exConsulta(Con, Query);
      extBarrio.next();
      extents.setExtension(extBarrio.getDouble("xmin"), extBarrio.getDouble("xmax"), extBarrio.getDouble("ymin"), extBarrio.getDouble("ymax"));
    } catch (SQLException x) 
    {
      x.printStackTrace();
      System.out.println(Query);
...
/*    xOffset = 0.0;
    yOffset = 0.0;
    Qwhere = " where within(the_geom,(select box3d(the_geom) from gis.barrios where " 
     + " idbarrio = public.barrios.idbarrio and public.barrios.nombre = '" 
     + comboBarrios.getSelectedItem().toString() +"' limit 1))";*/
...
//    puntoDestino.x + " " + puntoDestino.y + " 0)',-1)) ";
  ...
  public void zoomUbicacion()
  {
    ...
    { 
      ...
      double ubicX = Double.parseDouble(ubicacion.getText().substring(0,indexComa));
      double ubicY = Double.parseDouble(ubicacion.getText().substring(indexComa+1, ubicacion.getText().length()));
      String kCon = "select xmin(the_geom), xmax(the_geom), ymin(the_geom), ymax(the_geom) from  (select box3d(geometryfromtext('BOX3D(" + 
            (ubicX-100) + " " + (ubicY-100) + " 0," + 
            (ubicX+100) + " " + (ubicY+100) + " 0)',-1)) as the_geom) as foo";
      System.out.println(","+kCon);
      ResultSet extUbic = Proc.exConsulta(Con, kCon);
      extUbic.next();
      extents.setExtension(extUbic.getDouble("xmin"), extUbic.getDouble("xmax"), extUbic.getDouble("ymin"), extUbic.getDouble("ymax"));
    } catch (SQLException x) 
    {
      x.printStackTrace();
      System.out.println(Query);
...
//    puntoDestino.x + " " + puntoDestino.y + " 0)',-1)) ";
  ...
  {
    System.out.println(extents.getStrExtension());
    return extents;
  }
  
  public void setStrExtents(String _newExtent) 
  {
    extents.setStrExtension(_newExtent);
    Qwhere = " where within(the_geom,geometryfromtext('BOX3D(" + 
    extents.getMaxX() + " " + extents.getMaxY() + " 0," + 
    extents.getMinX() + " " + extents.getMinY() + " 0)',-1)) ";
    Qwhere = Qwhere + " or intersects(the_geom,geometryfromtext('BOX3D(" + 
    extents.getMaxX() + " " + extents.getMaxY() + " 0," + 
    extents.getMinX() + " " + extents.getMinY() + " 0)',-1)) ";
  }
