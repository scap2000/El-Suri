...
public class GaiaTiposAdminyServList extends BasicPrimitivePanel {
    private BasicPanel panelData = new BasicPanel();
    
    private int[] sizeColumnList = {41, 278};
    private Vector headerList = new Vector();
    private Vector dataRow = new Vector();
    private GridPanel listPanel = new GridPanel(30, sizeColumnList, "Tipos Administración/Servicios", dataRow);
    ...
    private GaiaTipoAdminyServ tipoAdminyServ;
    private boolean addAction = true;
    
    public GaiaTiposAdminyServList() {
	try {
	    jbInit();
	} catch (Exception e) {
	    e.printStackTrace();
	...
	panelData.setBorder(BorderPanel.getBorderPanel("Agregar nuevo Tipo de Administración o Servicio"));
	...
	listPanel.setParams("gis_oran.getAllTiposAdminyServ", "''", headerList);
    }
	...
	if (tipoAdminyServ == null){
	    tipoAdminyServ = new GaiaTipoAdminyServ(Integer.parseInt(""+ dataRow.elementAt(0)), dataRow.elementAt(1).toString());
	} else {
	    tipoAdminyServ.setIdActivity(Integer.parseInt(""+ dataRow.elementAt(0)));
	    tipoAdminyServ.setNombre(dataRow.elementAt(1).toString());
	}
    }
    
    private void loadData(){
	tfName.setValue(tipoAdminyServ.getNombre());
	addAction = false;
    }
	...
	if (addAction){
	    tipoAdminyServ = new GaiaTipoAdminyServ();
	}
	tipoAdminyServ.setNombre(tfName.getString());
	tipoAdminyServ.saveData();
