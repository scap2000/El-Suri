...
public class UpdateTgsFeesMgmt extends BasicPrimitivePanel {
    ...
    private TaxesTGS parentMain;

    public UpdateTgsFeesMgmt() {
	try {
	    jbInit();
	} catch (Exception e) {
	    e.printStackTrace();
    ...
    */
    ...
    private void getCuotas() {
        int resul = LibSQL.getInt("taxes.setTgsFees",cadastral.getIdCatastro());
    ...
    private void btnSaveData_actionPerformed(ActionEvent e) {
        if (Advisor.question("Aviso","¿Esta seguro de actualizar las cuotas?")) {
            ...
            for (int i = 0; i < aux.size(); i++) {
                ...
                if (LibSQL.getInt("taxes.updateTgsFees", params) <= 0) {
                    ok = false;
                }
            }
            if (ok) {
    ...
    public void setParentMain(TaxesTGS _parentMain) {
