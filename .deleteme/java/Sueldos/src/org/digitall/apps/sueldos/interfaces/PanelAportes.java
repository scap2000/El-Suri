...
public class PanelAportes extends BasicPrimitivePanel {
	...
	if(!contentResguardo.equals(contentActual)){
		...
		if(contentActual.save()){
		    refresh();
		    cargarContent(contentResguardo,false);
		    Advisor.messageBox("Se guardaron los cambios de pestaña APORTES","Datos grabados");
		}else{
	    ...
	    if(!_exit){
		Advisor.messageBox("No se realizaron cambios en la pestaña APORTES","Sin cambios");
	    }
	}
    }
