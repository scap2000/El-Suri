...
public class ResourcesQtyGraphics extends BasicPrimitivePanel{
    ...
    private BasicCheckBox chkRecibidos = new BasicCheckBox();

    public ResourcesQtyGraphics() {
	try {
	    jbInit();
	} catch (Exception e) {
	    e.printStackTrace();
	...
	btnDeleteResource.addActionListener(new ActionListener() {
        ...
        findPanel.add(chkRecibidos, null);
	...
	findPanel.setBorder(BorderPanel.getBorderPanel("Gráfico de recurso(s)"));
        ...
        chkRecibidos.setText("Facturados");
        chkRecibidos.setBounds(new Rectangle(15, 130, 115, 20));
            ...
            if (Proced.compareDates(Proced.setFormatDate(tfStartDate.getDate(),false),Proced.setFormatDate(tfEndDate.getDate(),false)) == 2) {
                ...
                if (idRecursosFavoritos.size() > 0) {
                    ...
                    for (int i = 0; i < idRecursosFavoritos.size();i++) {
                        String _params =  idRecursosFavoritos.elementAt(i) + "," + chkRecibidos.isSelected() + ",'" + Proced.setFormatDate(tfStartDate.getDate(), false) +"','"+ Proced.setFormatDate(tfEndDate.getDate(), false) +"', 0, 0";
                        ResultSet _results = LibSQL.exFunction("resourcescontrol.getmonthlyresourceqty", _params);
                        ...
                        try {
                            while (_results.next()) {
                                _serie.add(new Month(_results.getInt("month"), _results.getInt("year")), _results.getInt("qty"));
                            }
                    ...
                    JFreeChart _timeSeriesChart = ChartFactory.createTimeSeriesChart("Gráfico de recursos(s)", "Mes", "Cantidad", _dataset, true, true, false);
