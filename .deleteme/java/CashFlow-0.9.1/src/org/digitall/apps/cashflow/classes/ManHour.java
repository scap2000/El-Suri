...
public class ManHour extends Entity {

    private static final int entityType = EntityTypes.MAN_HOUR;
    /**
    ...
    public ManHour(double _amount) {
        super(entityType);
        manHour = new ManHour(_amount);
    }

    private boolean produceManHour(double _amount) {
        boolean _produce = false;
        if (doProduction(EntityTypes.MAN_HOUR, _amount)) {
            _produce = manHour.produce(_amount);
        }
        return _produce;
    }
