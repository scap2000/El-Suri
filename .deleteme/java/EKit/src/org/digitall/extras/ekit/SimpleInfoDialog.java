...
/** Class for providing a dialog that lets the user specify values for tag attributes.
  */
public class SimpleInfoDialog extends BasicDialog {
	...
	if (type == QUESTION) {
	    ...
	    jOptionPane = new JOptionPane(new BasicLabel(message, SwingConstants.CENTER), JOptionPane.PLAIN_MESSAGE, JOptionPane.OK_CANCEL_OPTION, null, buttonLabels, buttonLabels[0]);
	} else {
	    buttonLabels = new Object[] { Translatrix.getTranslationString("DialogClose") };
	    jOptionPane = new JOptionPane(new BasicLabel(message, SwingConstants.CENTER), JOptionPane.PLAIN_MESSAGE, JOptionPane.DEFAULT_OPTION, null, buttonLabels, buttonLabels[0]);
	}
