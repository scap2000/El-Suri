...
public final class SelectorFechaPanel extends JPanel {
	...
	for (int i = 0; i < 6; i++) {
		...
		if (days[k][i] == 0) {
		    ...
		    String anio =  yearCombo.getSelectedItem().toString().trim();		    
		    ...
		    //Aquí puse FALSE para poder seleccionar la fecha de hoy
	...
	String nro = "";
	if (Dia > 9) {
	    Q = "SELECT count(*) FROM tabs.holiday_tabs WHERE fecha = '" + Anio + "-" + Mes + "-" + Dia + "' AND descripcion <> 'Sabado' AND descripcion <> 'Domingo' ";
	} else {
	    Q = "SELECT count(*) FROM tabs.holiday_tabs WHERE fecha = '" + Anio + "-" + Mes + "-0" + Dia + "' AND descripcion <> 'Sabado' AND descripcion <> 'Domingo' ";
	}
	//System.out.println(Q);
	nro = org.digitall.lib.sql.LibSQL.getCampo(Q);
	if (nro.equals("0")) {
	    return false;
	} else {
	    return true;
