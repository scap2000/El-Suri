...
public class ConceptosSociales {
	...
	if (idconceptoreferencia == -1){
	    result = LibSQL.getInt("sueldos.addConceptoSociales",params);              
	} else {            
	    result = LibSQL.getInt("sueldos.setConceptoSociales",params);
	}               
	return result;
    }
	...
	ResultSet data = LibSQL.exFunction("sueldos.getConceptoSociales", params);
	try {
	    if (data.next()) {
