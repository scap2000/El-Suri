...
public class ConceptosFamiliares {
	...
	if (idconceptoreferencia == -1){
	    result = LibSQL.getInt("sueldos.addConceptoFamiliares",params);              
	} else {            
	    result = LibSQL.getInt("sueldos.setConceptoFamiliares",params);
	}               
	return result;
    }
	...
	ResultSet data = LibSQL.exFunction("sueldos.getConceptoFamiliares", params);
	try {
	    if (data.next()) {
