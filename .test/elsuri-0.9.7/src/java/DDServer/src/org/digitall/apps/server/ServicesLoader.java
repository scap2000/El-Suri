...
public class ServicesLoader {
	...
	checkCerts();
	File _config = new File("./logs.conf");
	System.out.println("Abriendo archivo de configuración: " + _config.getAbsolutePath());
	if (!_config.exists()) {
	    System.err.println("Error, no existe el archivo de configuración, se creará uno nuevo en la ubicación actual");
	    if (new File(System.getProperty("user.dir")).canWrite()) {
	        ...
	        confLogs.setProperty("bdlocal","logsdatabase");
	        ...
	        confLogs.setProperty("activo","no");
	        System.err.println("Modifique el archivo logs.conf a su gusto e inicie la aplicación");
	        System.exit(0);
	    } else {
    ...
    public void checkCerts() {
        if ( (SSLConfig.class.getResourceAsStream(SSLConfig.KEY_FILE_PATH) != null) &&
    	    ...
    	    System.err.println("No se han encontrado los certificados SSL");
            System.exit(-1);
        }
    }
