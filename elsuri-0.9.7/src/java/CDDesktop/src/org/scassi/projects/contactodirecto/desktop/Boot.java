package org.scassi.projects.contactodirecto.desktop;
...
import org.scassi.projects.contactodirecto.ReportesMain;
...
public class Boot {
	...
	Thread _thread = new Thread(new Runnable() {
		...
		try {
		    ...
		    //GeometrySet gsComercios = new GeometrySet("gis", "padroncomercial", "the_geom", "1 = 1", "idpadron");
		    GeometrySet gsAlbergues = new GeometrySet("gis", "padroncomercial", "the_geom", "1 = 1", "idpadron");
		    gsAlbergues.getGeometrySetConfig().getGeometryTypeFromSQL(null);
		    gsAlbergues.setParent(mapCanvas);
		    gsAlbergues.reload();

		    Layer lyAlbergues = new Layer("Albergues", gsAlbergues,"gid","gid","gid");
		    lyAlbergues.setColor(Color.black);
		    lyAlbergues.setFillColor(Color.blue);
		    lyAlbergues.setMouseOverColor(Color.ORANGE);
                    lyAlbergues.setPointDiameter(14);
                    
                    CommercesMgmt _infoPanel = new CommercesMgmt();
                    lyAlbergues.setInfoPanel(_infoPanel);
		    lyAlbergues.setOn();

                    UpdateMessageBroker.informSubscribers();

		    LayerGroup layerGroup = new LayerGroup("Reports");

		    mapCanvas.addLayerGroup(layerGroup);
		    layerGroup.add(lyAlbergues);
		    mapCanvas.addLayer(lyAlbergues);

		    Vector<GeometrySet> geometrySets = new Vector<GeometrySet>();
		    geometrySets.add(gsAlbergues);
		    mapCanvas.setGeometrySets(geometrySets);
		    mapCanvas.setMapExtents(-65.4231976, -24.7821269, -65.5006691, -24.7960685);
		    JInternalFrame layerListFrame = new JInternalFrame("Layer list");
		    ...
		    _desktop.add(layerListFrame);
                    ...
                    //Aquí recién puedo agregar el coordinateViewer
		    CoordinateViewer coordinateViewer = new CoordinateViewer();
		    coordinateViewer.setClosable(false);
		    coordinateViewer.setTitle("Coordinates");
		    _desktop.add(coordinateViewer);
		    mapCanvas.setCoordinateViewer(coordinateViewer);
                    mapCanvas.setRuleViewer(ruleViewer);
		    ...
		    mapBasicToolsFrame.setDrawPanel(mapCanvas);
		    mapBasicToolsFrame.show();

		    ExtendedInternalFrame reportesMainFrame = new ExtendedInternalFrame("Mobile Reports");
		    ReportesMain reportesMain = new ReportesMain(mapCanvas, layerGroup);
		    reportesMainFrame.setClosable(false);
		    reportesMainFrame.setIconifiable(false);
		    reportesMainFrame.setCentralPanel(reportesMain);
		    reportesMainFrame.setDesktop(Environment.getDesktop(Environment.MODULE_GAIA));
		    reportesMainFrame.show();
		    ComponentsManager.setConfigurable(reportesMainFrame);
		    ...
		    layerTree.setDrawEnginePanel(mapCanvas);
