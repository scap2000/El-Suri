...
public class UnicodeDialog extends BasicDialog implements ActionListener {
...
*/
    ...
    private BasicCombo jcmbBlockSelector;
    private BasicCombo jcmbPageSelector;
	...
	centerPanel.add(new BasicLabel(""));
	for (int labelLoop = 0; labelLoop < 16; labelLoop++) {
	    BasicLabel jlblMarker = new BasicLabel("x" + (labelLoop > 9 ? "" + (char)(65 + (labelLoop - 10)) : "" + labelLoop));
	...
	for (int counter = 0; counter < UNICODEBLOCKSIZE; counter++) {
	    if ((counter % 16) == 0) {
		BasicLabel jlblMarker = new BasicLabel((labelcount > 9 ? "" + (char)(65 + (labelcount - 10)) : "" + labelcount) + "x");
		jlblMarker.setHorizontalAlignment(SwingConstants.CENTER);
		jlblMarker.setVerticalAlignment(SwingConstants.CENTER);
		jlblMarker.setForeground(new Color(0.5f, 0.5f, 0.75f));
		centerPanel.add(jlblMarker);
	...
	BasicPanel selectorPanel = new BasicPanel();
	jcmbBlockSelector = new BasicCombo(unicodeBlocks);
	...
	String[] sPages = { "1" };
	jcmbPageSelector = new BasicCombo(sPages);
	...
	selectorPanel.add(new BasicLabel(Translatrix.getTranslationString("SelectorToolUnicodeBlock")));
	selectorPanel.add(jcmbBlockSelector);
	selectorPanel.add(new BasicLabel(Translatrix.getTranslationString("SelectorToolUnicodePage")));
	selectorPanel.add(jcmbPageSelector);
	BasicPanel buttonPanel = new BasicPanel();
	BasicButton closeButton = new BasicButton(Translatrix.getTranslationString("DialogClose"));
