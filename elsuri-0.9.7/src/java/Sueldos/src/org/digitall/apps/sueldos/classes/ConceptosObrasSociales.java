...
public class ConceptosObrasSociales {
	...
	if (idconceptoreferencia == -1){
	    result = LibSQL.getInt("sueldos.addConceptoObrasSociales",params);              
	} else {            
	    result = LibSQL.getInt("sueldos.setConceptoObrasSociales",params);
	}               
	return result;
    }
	...
	ResultSet data = LibSQL.exFunction("sueldos.getConceptoObrasSociales", params);
	try {
	    if (data.next()) {
