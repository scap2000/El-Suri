...
public class ResourcesMgmt extends BasicPrimitivePanel {
    ...
    private BasicPanel dataPanel = new BasicPanel("Agregar/Modificar un Recurso");
    private CBInput cbLifeTimeTypes = new CBInput(CachedCombo.LIFETIMETYPES, "LifeTimeTypes");
    private TFInput tfLifeTime = new TFInput(DataTypes.STRING, "LifeTime", false);
    private BasicPanel brandsPanel = new BasicPanel("Marcas Asignadas");
    ...
    private TFInput tfMinStock = new TFInput(DataTypes.DOUBLE, "MinStock", false);
    private AddButton btnNewAssign = new AddButton();
    private BasicPanel content = new BasicPanel();    
    private RecursosDistinguiblesPanel recursosDistinguiblesPanel = null;
	...
	//this.setLayout( null );
	this.setSize(new Dimension(565, 510));
	...
	tfDescription.setBounds(new Rectangle(15, 145, 525, 35));
    ...
    private void listPanel_mouseClicked(MouseEvent e) {
	if (e.getClickCount() == 2) {            
	    recursosDistinguiblesPanel = new RecursosDistinguiblesPanel();
	    ExtendedInternalFrame stockMgmtContainer = new ExtendedInternalFrame("Agregar / Modificar");
	    recursosDistinguiblesPanel.setIdMarca(Integer.parseInt(listPanel.getValuesAt(2).get(listPanel.getTable().getSelectedRow()).toString()));
	    recursosDistinguiblesPanel.setIdRecurso(Integer.parseInt(parentList.getListPanel().getValuesAt(0).get(parentList.getListPanel().getTable().getSelectedRow()).toString()));	    	    
	    recursosDistinguiblesPanel.setNombreTipoVidaUtil(cbLifeTimeTypes.getSelectedItem().toString());
	    recursosDistinguiblesPanel.setIdTipoVidaUtil(Integer.parseInt(cbLifeTimeTypes.getSelectedValue().toString()));
	    recursosDistinguiblesPanel.refresh();
	    stockMgmtContainer.setCentralPanel(recursosDistinguiblesPanel);   
