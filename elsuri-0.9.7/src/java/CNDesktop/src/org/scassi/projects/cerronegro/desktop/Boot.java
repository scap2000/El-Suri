...
import org.digitall.common.geo.mapping.AerialDrawEngine;
import org.digitall.common.geo.mapping.BasicDrawEngine;
import org.digitall.common.geo.mapping.OSMDrawEngine;
...
import org.digitall.common.geo.mapping.osm.MapMarkerDot;
import org.digitall.common.geo.mapping.osm.OsmFileCacheTileLoader;
import org.digitall.common.geo.mapping.osm.OsmTileLoader;
...
public class Boot {
	...
	Thread _thread = new Thread(new Runnable() {
		...
		try {
		    /**
		    * 1º GaiaEnvironment.initialize()
		    * 2º Crear DrawEngine
		    * 3º if (!GaiaEnvironment.gaiaEngine.initialize()) {
		    *  4º GaiaEnvironment.setScheme("gis");
		    *  5º Crear geometrySets
		    *  6º Crear Layers
		    *  7º Crear grupos de layers y agregarlo: GaiaEnvironment.layerGroups.add(mainGroup);
		    *  8º GaiaEnvironment.gaiaEngine.setLayerGroupList(GaiaEnvironment.layerGroups);
		    *  9º GaiaEnvironment.geometrySets = geometrySets;
		    *  10º Ciclo de carga de los LayerGroups
		    * }
		    * 11º cityMap.initialize();
		    */
		    GaiaEnvironment.initialize();//línea agregada para cargar la configuración de GAIA

		    final OSMDrawEngine cityMap = new OSMDrawEngine();
		    cityMap.setMapExtents(3556114.5299, 7260975.3743, 3556551.2590, 7261336.3605);
		    cityMap.setProjectionType(CoordinateSystems.LL);

		    if (!GaiaEnvironment.gaiaEngine.initialize()) {//línea agregada para cargar la configuración de GAIA
		        GaiaEnvironment.setScheme("gis");
		        GeometrySet gsetLotes = new GeometrySet("lotes", "the_geom", "1=1", "mapkey");
			gsetLotes.getGeometrySetConfig().setProjectionType(CoordinateSystems.LL);
			Layer lotes = new Layer("Lotes", gsetLotes);
		        lotes.setColor(Color.MAGENTA);
			lotes.setOn();
		        lotes.setMouseOverColor(Color.ORANGE);
		    ...
		    button.addActionListener(new ActionListener() {
		        ...
		        public void actionPerformed(ActionEvent e) {
		            cityMap.setDisplayToFitMapMarkers();
		        }
		    });
		    ...
		    tileSourceSelector.addItemListener(new ItemListener() {
		        public void itemStateChanged(ItemEvent e) {
		            try {
		                cityMap.setTileSource((TileSource) e.getItem());
		            } catch  (Exception x) {
		    ...
		    try {
		        tileLoaderSelector = new JComboBox(new TileLoader[] { new OsmFileCacheTileLoader(cityMap),
		                new OsmTileLoader(cityMap) });
		    } catch (IOException e) {
		        tileLoaderSelector = new JComboBox(new TileLoader[] { new OsmTileLoader(cityMap) });
		    }
		    tileLoaderSelector.addItemListener(new ItemListener() {
		        public void itemStateChanged(ItemEvent e) {
		            cityMap.setTileLoader((TileLoader) e.getItem());
		        }
		    });
		    cityMap.setTileLoader((TileLoader) tileLoaderSelector.getSelectedItem());
		    //_mapViewer.setTileSource((TileSource)tileSourceSelector.getSelectedItem());
		    panel.add(tileSourceSelector);
		    panel.add(tileLoaderSelector);
		    final JCheckBox showMapMarker = new JCheckBox("Map markers visible");
		    showMapMarker.setSelected(cityMap.getMapMarkersVisible());
		    showMapMarker.addActionListener(new ActionListener() {

		        public void actionPerformed(ActionEvent e) {
		            cityMap.setMapMarkerVisible(showMapMarker.isSelected());
		        }
		    });
		    panel.add(showMapMarker);
		    final JCheckBox showTileGrid = new JCheckBox("Tile grid visible");
		    showTileGrid.setSelected(cityMap.isTileGridVisible());
		    showTileGrid.addActionListener(new ActionListener() {

		        public void actionPerformed(ActionEvent e) {
		            cityMap.setTileGridVisible(showTileGrid.isSelected());
		        }
		    });
		    panel.add(showTileGrid);
		    final JCheckBox showZoomControls = new JCheckBox("Show zoom controls");
		    showZoomControls.setSelected(cityMap.getZoomContolsVisible());
		    showZoomControls.addActionListener(new ActionListener() {

		        public void actionPerformed(ActionEvent e) {
		            cityMap.setZoomContolsVisible(showZoomControls.isSelected());
		        }
		    });
		    panel.add(showZoomControls);
		    panel.add(button);

		    cityMap.addMapMarker(new MapMarkerDot(Color.GREEN, -24.78924754938909, -65.41025340557098));
		    cityMap.setDisplayPositionByLatLon(-24.78924754938909, -65.41025340557098, 14); //Salta
		    
		    //_mapViewer.addMapMarker(new MapMarkerDot(Color.GREEN, -22.59620734990284, -63.81101846694946));
		    //_mapViewer.setDisplayPositionByLatLon(-22.59620734990284, -63.81101846694946, 14); //Mosconi

		    // _mapViewer.setTileGridVisible(true);
		    
		    cityMap.setBounds(_desktop.getBounds());
		    cityMap.setLayout(new BorderLayout());
		    cityMap.add(panel, BorderLayout.NORTH);
		    //cityMap.add(helpPanel, BorderLayout.SOUTH);

		    cityMap.addMapMarker(new MapMarkerDot(Color.CYAN, -24.994747260332176, -65.4468432747274));
		    cityMap.addMapMarker(new MapMarkerDot(Color.ORANGE, -25.01834287563326, -65.42350533063109));
		    ...
		    layerTree.setDrawEnginePanel(cityMap);
