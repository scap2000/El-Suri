...
public class TaxesAdminMgmt extends BasicPrimitivePanel {
    ...
    private CBInput cbCtaDeduccion = new CBInput(0,"Cuenta Deducción", false);
    private CBInput cbCtaInteres = new CBInput(0,"Cuenta Interés", false);
	...
	this.setTitle("Administración de Impuestos");
	...
	headerList.addElement("*");
	headerList.addElement("Cuenta Deducción");
	headerList.addElement("*");
	headerList.addElement("Cuenta Interés");
	...
	if (Advisor.question("Aviso","¿Está seguro de borrar el impuesto \""+ tipoIpuesto.getNombre() +"\"?")) {
	    if (tipoIpuesto.deleteData()) {
		clearFields();
		refresh();
	    } else  {
		Advisor.messageBox("Ocurrió un error, no se pudo borrar el Impuesto seleccionado", "Mensaje");
	    }
	}
    }
	...
	if (control()) {
	    ...
	    if (error == NOMBRE) {
	        Advisor.messageBox("El campo \"Impuesto\" no debe estar vacío","Mensaje");
	        ...
	        Advisor.messageBox("Debe seleccionar una cuenta para la Deducción","Mensaje");
	    } else  if (error == CBINTERES) {
	        Advisor.messageBox("Debe seleccionar una cuenta para el Interés","Mensaje");
	    }
	}
    }
