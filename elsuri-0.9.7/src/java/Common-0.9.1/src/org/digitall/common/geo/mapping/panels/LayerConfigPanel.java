	...
	*/
    ...
    private void cbGroupName_itemStateChanged(ItemEvent e) {
        if (cbGroupName.getSelectedIndex() > -1) {
            //moveLayerToGroup(layer.getLayerConfig().getGroupName(), layer.getName(), cbGroupName.getSelectedItem().toString());
        }
    }
	...
	for (int i = 0; i < layerGroups.size(); i++) {
	    ...
	    cbGroupName.getCombo().addItem(layerGroups.elementAt(i).getName(),i);
	}
    }
}
