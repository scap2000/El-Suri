...
MouseWheelListener {
	...
	/*************** FORMA PRECISA DE OBTENER COORDENADAS **********/
	Point screenPoint = e.getLocationOnScreen();
	int x = map.getCenter().x + e.getX() - map.getWidth() / 2;
	int y = map.getCenter().y + e.getY() - map.getHeight() / 2;
	double lon = OsmMercator.XToLon(x, map.getZoom());
	double lat = OsmMercator.YToLat(y, map.getZoom());
	Coordinate c = new Coordinate(lat, lon);
	System.out.println("LatLon: " + c.getLat() + ", " + c.getLon() );
    }
	...
	double lon = OsmMercator.XToLon(x, map.getZoom());
	double lat = OsmMercator.YToLat(y, map.getZoom());
	map.getCoordinateViewer().setLLCoord(new LatLongCoord(lat, lon));
    }
