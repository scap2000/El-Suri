...
public class ConceptosCategorias {
	...
	if (idconceptoreferencia == -1){
	    result = LibSQL.getInt("sueldos.addConceptoCategorias",params);              
	} else {            
	    result = LibSQL.getInt("sueldos.setConceptoCategorias",params);
	}               
	return result;
    }
	...
	ResultSet data = LibSQL.exFunction("sueldos.getConceptoCategorias", params);
	try {
	    if (data.next()) {
