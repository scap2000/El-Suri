...
public class PanelReporteServiciosEjecutados extends BasicPrimitivePanel {

    private Vector dataRow = new Vector();
    private GridPanel listPanel = new GridPanel(30, new int[] { 210, 61, 186, 70, 75, 75, 60 }, "Servicios Ejecutados", dataRow);
    ...
    private CBInput cbCategoria = new CBInput(0, "Categoria", false);
    ...
    public PanelReporteServiciosEjecutados() {
	try {
	    jbInit();
	} catch (Exception e) {
	    e.printStackTrace();
	...
	cbCategoria.setBounds(new Rectangle(400, 20, 195, 35));
	...
	jpCenter.add(cbCategoria, null);
	jpCenter.add(btnFind, null);
	jpCenter.add(listPanel, null);
	this.add(jpCenter, BorderLayout.CENTER);
	cbCategoria.setGeneralItem(true);
	cbCategoria.loadJCombo("taller.getAllCategorias", "''" + ",0,0");
	...
	headerList.addElement("Servicio");
	...
	listPanel.setParams("taller.getReporteServiciosEjecutados", "'', '', '', '', -1", headerList);
    }

    public void refresh() {
	listPanel.refresh(tfFechaDesde.getDateForSQL() + "," + tfFechaHasta.getDateForSQL()  + "," + tfFechaFacturadoDesde.getDateForSQL() + "," + tfFechaFacturadoHasta.getDateForSQL() + "," + cbCategoria.getSelectedValue());
    }
