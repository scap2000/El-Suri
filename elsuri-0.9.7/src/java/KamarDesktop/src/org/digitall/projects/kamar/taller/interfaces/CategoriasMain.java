...
import org.digitall.projects.kamar.taller.classes.Categoria;

public class CategoriasMain extends BasicPrimitivePanel {

    private Vector headerList = new Vector();
    private Vector dataRow = new Vector();
    private GridPanel listPanel = new GridPanel(30, new int[] {315}, "Categorías", dataRow);
	...
	dataPanel.setBorder(BorderPanel.getBorderPanel("Agregar/Modificar categoría", Color.blue, Color.black));
	setHeaderList();
    }
	...
	listPanel.setParams("taller.getAllCategorias", "''", headerList);
    }
	...
	if (dataRow.size() > 0) {
	    categoria = new Categoria(new Integer(dataRow.elementAt(0).toString()), dataRow.elementAt(1).toString());
	    tfNombre.setValue(categoria.getNombre());
	...
	if (blank){
	    ...
	    _continue = Advisor.question("Agregar/Modificar categoría", "¿Está seguro de modificar la categoría \"" + categoria.getNombre() + "\"?");
	}
	if (_continue) {
	    if (tfNombre.getString().length() == 0) {
	        Advisor.messageBox("Debe ingresar un nombre para la categoría", "Nombre vacío");
	    } else {
		categoria.setNombre(tfNombre.getString());
		if (categoria.saveData() != -1) {
		    clearData();
		    refresh();
		    _returns = true;
		} else {
		    Advisor.messageBox("Ha ocurrido un error al intentar grabar los datos", "Agregar/Modificar categoría");
		}
	    }
	}
	return _returns;
	...
	if (dataRow.size() > 0) {
	    if (Advisor.question("Remover Categoría", "¿Está seguro de remover la categoría \"" + dataRow.elementAt(1) + "\"?")) {
		if (LibSQL.getBoolean("taller.removecategoria", dataRow.elementAt(0))) {
		    clearData();
		    refresh();
		} else {
		    Advisor.messageBox("Ha ocurrido un error al intentar remover la categoría", "Remover categoría");
		}
	    }
	}
    }
