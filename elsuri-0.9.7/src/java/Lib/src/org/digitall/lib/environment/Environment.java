...
public abstract class Environment {
    ...
    public static ConfigFile cfg = new ConfigFile(System.getProperty("user.home") + File.separator + "ddesktop.conf");
    public static ConfigFile defaultCfg = new ConfigFile(System.getProperty("user.home") + File.separator + "default.conf");
    ...
    public static String developer = "SCasSI";
    ...
    public static String MODULE_AVL = "<html><p align=center>Seguimiento en tiempo real</html>";
    ...
    public static String mediaServer = "http://172.20.30.210";
	...
	try {
		...
		serverDate = timeRecord.getString("now");
	    }
	...
	if (!(localTime.get(Calendar.DAY_OF_MONTH) == serverTime.get(Calendar.DAY_OF_MONTH) && localTime.get(Calendar.MONTH) == serverTime.get(Calendar.MONTH) && localTime.get(Calendar.YEAR) == serverTime.get(Calendar.YEAR))) {
	    if (Advisor.question("Sistema de fechas incorrecto", "La fecha de su computadora es: " + Calendar.getInstance().getTime().toString() + "\n y la del servidor es: " + serverDate + "\n¿Desea continuar a pesar del error?")) {
	    } else {
	        System.exit(0);
	    }
